    ...
    <div className="w-full min-h-screen flex items-center justify-center pt-[100px] px-margin-mobile relative z-10">
      <div className="w-full max-w-[450px] glass-panel p-8 md:p-10 rounded-2xl border border-outline-variant/20 shadow-2xl">
        <div className="text-center mb-10 select-none">
          <h1 className="font-display-xl text-[32px] text-on-surface italic mb-2 tracking-tight">{t('accessPortal')}</h1>
          <p className="font-body-md text-on-surface-variant text-[14px]">{t('authSubtitle')}</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center">
              ...
              <button type="button" className="text-[10px] text-primary uppercase tracking-widest hover:underline">{t('forgotPass')}</button>
            </div>
            <input 
              type="password" 
              value={password}
        ...
        <div className="mt-8 pt-8 border-t border-outline-variant/10 text-center select-none">
          <p className="font-body-md text-[13px] text-on-surface-variant">
            {t('newToCommunity')} <Link to="/register" className="text-primary font-bold hover:underline">{t('createAccount')}</Link>
          </p>
        </div>
      </div>
    </div>
